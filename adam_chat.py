...
Please answer the current question, using the context if relevant."""
            ...
            if self.stream_output:
                ...
                response_chunks = []
                if RICH_AVAILABLE:
                    console.print(f"[bold green]ADAM ({model}):[/bold green] ", end="")
                else:
                    print(f"\nADAM ({model}): ", end="", flush=True)
...
Available models depend on your API keys:
- Grok models (grok-4, grok-3-mini) - Set XAI_API_KEY
- OpenAI models (gpt-4, gpt-3.5-turbo) - Set OPENAI_API_KEY
