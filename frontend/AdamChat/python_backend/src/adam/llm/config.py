...
class LLMConfig:
    ...
    def __init__(self):
        ...
        # Additional API keys from ADAM format
        if not self.api_keys[ModelProvider.OPENAI]:
            self.api_keys[ModelProvider.OPENAI] = os.getenv("OPENAI_API_KEY")
        if not self.api_keys[ModelProvider.GROK]:
            self.api_keys[ModelProvider.GROK] = os.getenv("XAI_API_KEY")
