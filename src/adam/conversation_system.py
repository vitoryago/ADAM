...
class ConversationSystem:
    ...
    def record_exchange(self, query: str, response: str, 
        ...
        # Update topic index
        for topic in topics:
            if self.current_session.session_id not in self.topic_to_sessions[topic]:
                self.topic_to_sessions[topic].append(self.current_session.session_id)
    ...
    def end_session(self) -> Optional[str]:
        ...
        if not self.current_session.title and self.current_session.topics:
            # Use most common topics for title
