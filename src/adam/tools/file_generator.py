...
class FileGenerator:
    ...
    def _load_templates(self) -> Dict[str, str]:
        ...
        return {
            ...
            # New SQL templates
            'sql_query': self._sql_query_template(),
            'snowflake_query': self._snowflake_query_template(),
            'sql_analysis': self._sql_analysis_template(),
            'dbt_model': self._dbt_model_template(),
            'sql_stored_procedure': self._sql_stored_procedure_template()
        }
    ...
    def generate(self, file_type: str, name: str, **kwargs) -> GeneratedFile:
        ...
        if file_type == 'python_script':
            ...
            content = self._generate_sql_query(name, **kwargs)
            filename = f"{self._to_snake_case(name)}.sql"
        elif file_type == 'snowflake_query':
            content = self._generate_snowflake_query(name, **kwargs)
            filename = f"{self._to_snake_case(name)}_snowflake.sql"
        elif file_type == 'sql_analysis':
            content = self._generate_sql_analysis(name, **kwargs)
            filename = f"{self._to_snake_case(name)}_analysis.sql"
        elif file_type == 'dbt_model':
            content = self._generate_dbt_model(name, **kwargs)
            filename = f"{self._to_snake_case(name)}.sql"
        elif file_type == 'sql_stored_procedure':
            content = self._generate_sql_stored_procedure(name, **kwargs)
            filename = f"sp_{self._to_snake_case(name)}.sql"
        else:
...
'''
    ...
    def _generate_sql_query(self, name: str, query_type: str = 'select', **kwargs) -> str:
        """Generate SQL query template"""
        date_str = datetime.now().strftime("%Y-%m-%d")
        
        if query_type == 'select':
            template = f"""-- {name} Query
-- Generated by ADAM on {date_str}
-- Purpose: {kwargs.get('purpose', 'Data retrieval query')}
...
WHERE 
    condition = 'value'
    AND date_column >= CURRENT_DATE - 30
ORDER BY 
    column1 DESC
LIMIT 1000;
"""
        elif query_type == 'aggregate':
            template = f"""-- {name} Aggregation Query
-- Generated by ADAM on {date_str}
-- Purpose: {kwargs.get('purpose', 'Data aggregation and analysis')}
...
"""
    ...
    def _generate_snowflake_query(self, name: str, **kwargs) -> str:
        """Generate Snowflake-specific query"""
        date_str = datetime.now().strftime("%Y-%m-%d")
        warehouse = kwargs.get('warehouse', 'COMPUTE_WH')
        database = kwargs.get('database', 'MY_DATABASE')
        schema = kwargs.get('schema', 'PUBLIC')
        
        template = f"""-- {name} Snowflake Query
-- Generated by ADAM on {date_str}
-- Snowflake-specific optimized query
...
-- Main query with Snowflake features
WITH base_data AS (
    SELECT 
        *,
...
aggregated AS (
    SELECT 
        DATE_TRUNC('day', timestamp_col) as date,
        category,
        COUNT(*) as count,
...
"""
    ...
    def _generate_sql_analysis(self, name: str, **kwargs) -> str:
        """Generate comprehensive SQL analysis query"""
        date_str = datetime.now().strftime("%Y-%m-%d")
        table_name = kwargs.get('table', 'target_table')
        
        template = f"""-- {name} Data Analysis Query Suite
-- Generated by ADAM on {date_str}
-- Comprehensive data quality and analysis queries
...
SELECT 
    ...
    LAG(COUNT(*), 1) OVER (ORDER BY DATE_TRUNC('day', timestamp_col)) as prev_day_count,
    ROUND((COUNT(*) - LAG(COUNT(*), 1) OVER (ORDER BY DATE_TRUNC('day', timestamp_col))) * 100.0 / 
          NULLIF(LAG(COUNT(*), 1) OVER (ORDER BY DATE_TRUNC('day', timestamp_col)), 0), 2) as pct_change
FROM {table_name}
WHERE timestamp_col >= CURRENT_DATE - 30
GROUP BY DATE_TRUNC('day', timestamp_col)
ORDER BY date DESC;
...
SELECT 
    *,
    ABS(value - stats.mean_val) / NULLIF(stats.stddev_val, 0) as z_score
FROM {table_name}
CROSS JOIN stats
WHERE ABS(value - stats.mean_val) / NULLIF(stats.stddev_val, 0) > 3
ORDER BY z_score DESC
LIMIT 100;
"""
        return template
    
    def _generate_dbt_model(self, name: str, **kwargs) -> str:
        """Generate dbt model SQL file"""
        date_str = datetime.now().strftime("%Y-%m-%d")
        model_name = self._to_snake_case(name)
        
        template = f"""-- {model_name}.sql
-- dbt model generated by ADAM on {date_str}
...
SELECT 
    {{{{ dbt_utils.surrogate_key(['id', 'date_day']) }}}} as unique_key,
    *
FROM enriched
"""
        return template
    
    def _generate_sql_stored_procedure(self, name: str, **kwargs) -> str:
        """Generate SQL stored procedure"""
        date_str = datetime.now().strftime("%Y-%m-%d")
        proc_name = self._to_snake_case(name).upper()
        
        template = f"""-- Stored Procedure: {proc_name}
-- Generated by ADAM on {date_str}
...
RETURNS TABLE(result_date DATE, result_category VARCHAR, result_value NUMBER)
LANGUAGE SQL
AS
$$
DECLARE
...
BEGIN
    ...
    IF (start_date > end_date) THEN
        RETURN TABLE(NULL, 'ERROR', -1);
    END IF;
    
    -- Create temporary result table
    CREATE OR REPLACE TEMPORARY TABLE temp_results AS
    SELECT 
        date_column as result_date,
        category_column as result_category,
        SUM(value) as result_value
    FROM 
        source_table
    WHERE 
        date_column BETWEEN :start_date AND :end_date
        AND (category_column = :category OR :category IS NULL)
    GROUP BY 
        date_column, category_column;
    
    -- Get row count for logging
    SELECT COUNT(*) INTO row_count FROM temp_results;
    
    -- Log execution
    INSERT INTO procedure_log (procedure_name, execution_time, row_count, parameters)
    VALUES ('{proc_name}', CURRENT_TIMESTAMP(), :row_count, 
            OBJECT_CONSTRUCT('start_date', :start_date, 'end_date', :end_date, 'category', :category));
    
    -- Return results
    RETURN TABLE(SELECT * FROM temp_results ORDER BY result_date DESC);
...
EXCEPTION
    WHEN OTHER THEN
        ...
        VALUES ('{proc_name}', :status_message, CURRENT_TIMESTAMP());
        RAISE;
END;
$$;

-- Grant execution permissions
GRANT USAGE ON PROCEDURE {proc_name}(DATE, DATE, VARCHAR) TO ROLE analyst_role;

-- Example usage:
-- CALL {proc_name}('2024-01-01', '2024-12-31', 'Electronics');
"""
        return template
    ...
    def _sql_query_template(self) -> str: return ""
    def _snowflake_query_template(self) -> str: return ""
    def _sql_analysis_template(self) -> str: return ""
    def _dbt_model_template(self) -> str: return ""
    def _sql_stored_procedure_template(self) -> str: return ""
