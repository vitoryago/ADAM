...
class CostMonitor:
    ...
    def __init__(self, storage_path: str = "./cost_tracking"):
        ...
        # Alerts
        self.alerts = {
            "high_cost_query": 0.10,  # Alert if single query > $0.10
            "daily_warning": 0.80,    # Warn at 80% of daily budget
            "daily_limit": 1.00,      # Hard stop at daily limit
            "hourly_spike": 0.25      # Alert if any hour exceeds $0.25
        }
    ...
    def _check_alerts(self, query_data: Dict):
        ...
        if query_data["cost"] > self.alerts["high_cost_query"]:
            logger.warning(f"⚠️ High cost query: ${query_data['cost']:.4f} for {query_data['model']}")
        ...
        if daily_total >= daily_limit:
            ...
            # In production, this would disable expensive models
        elif daily_total >= self.alerts["daily_warning"]:
            remaining = daily_limit - daily_total
            logger.warning(f"⚠️ Approaching daily limit: ${remaining:.4f} remaining")
        ...
        if hour_cost > self.alerts["hourly_spike"]:
            logger.warning(f"⚠️ High hourly spend: ${hour_cost:.4f} in current hour")
    ...
    def generate_cost_report(self, days: int = 7) -> Dict:
        ...
        for i in range(days):
            ...
            if file_path.exists():
                ...
                for query in data["queries"]:
                    if query["cost"] > 0.05:  # Queries over $0.05
        ...
        # Sort expensive queries by cost
        report["expensive_queries"].sort(key=lambda x: x["cost"], reverse=True)
        report["expensive_queries"] = report["expensive_queries"][:10]  # Top 10
...
async def demo_cost_monitoring():
    ...
    for query, model, in_tokens, out_tokens, cost in test_queries:
        monitor.track_query(query, model, in_tokens, out_tokens, cost, 1.5)
    ...
    for key, value in status.items():
        if isinstance(value, float):
            print(f"  {key}: ${value:.4f}" if 'cost' in key or 'total' in key else f"  {key}: {value:.2f}")
        else:
            print(f"  {key}: {value}")
    
    # Generate visualization
    monitor.visualize_costs("demo_cost_dashboard.png")
    print("\nCost dashboard saved to demo_cost_dashboard.png")
